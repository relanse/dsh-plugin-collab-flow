...
| M4 | 安装/卸载、兼容性回归、发行包与发布文档 | 已完成发行准备 | `93f26ad`、`cbca0c7` |
...
新增 CLI 的协议解析、成功结算、取消、防委派与 usage 均通过确定性测试；原生模型成功调用的两项环境限制不能视为已通过。完整记录见 [M3 验收记录](./m3-verification.md)。M3 交付时 M4 尚待开始；M4 结果见下节。M3 未进行 npm 发布或重复完整 Web 浏览器验收。

## M4 完成内容

- 修复协作图 Bundle 缺失；默认 Web profile 可加载图与模板库，工作流引擎仅在运行模板时检查。
- 修复公共客户端声明中的 CSS 引用，精简两包发布文件并补齐许可证。
- 增加 release:check、release:pack、test:install；产物记录源 commit、dirty 状态和 SHA-256。
- 真实发行包在独立 DSH profile 中完成安装、重启、部分卸载、完全卸载和重装恢复。
- 覆盖 provider 卸载后的注册行为与已接纳句柄所有权，保留持久化数据，不自动清除用户数据。
- 新增 Windows CI 和发行/回滚/兼容文档；未配置自动 npm 发布。

## M4 验收记录

| 检查 | 结果 |
|---|---|
| 完整门禁 | 冷构建、98 项测试、类型检查、lint、冻结安装全部通过 |
| 干净提交候选包 | cbca0c7，dirty=false；CLI 29 个文件、协作图 24 个文件 |
| 真实安装生命周期 | 五阶段全部通过；模板、CLI 记录和协作图跨重启与重装恢复 |
| 可选依赖 | 移除 CLI 后图服务继续工作；默认 Web 无工作流引擎时图与模板库仍可用 |
| 临时环境 | 独立 DSH_HOME、本机随机端口；无模型调用，临时 profile 已清理 |
| 发布状态 | 已生成可审阅发行包，未执行 npm publish |

实现提交为 `93f26ad` 和 `cbca0c7`。详细摘要和复现记录见 [M4 验收记录](./m4-verification.md)，部署流程见 [发行指南](../release/README.md)。M0–M4 的工程阶段已完成；原生 Claude/Codex 成功模型调用与正式 registry 发布仍按各自验证条件单独处理。
...
- [发行与安装指南](../release/README.md)
- [发行验收深挖](../deep-dive/11-release-verification.md)
- [M4 验收记录](./m4-verification.md)
